...
#definition[Nichols Chart][
  A Nichols chart is a plot of $20 log_(10) |G(j omega)|$ (in dB) versus $angle(G(j omega))$ (degrees) with frequency $omega$ as a parameter.
]

#definition[Log-Magnitude Coordinates][
  The log-magnitude is expressed in decibels:
  $ M = 20 log_(10) |G(j omega)| $ (in dB)
...
#example[First-Order System][
  For $G(s) = 1/(tau s + 1)$:
  At $omega = 1/tau$: $|G(j omega)| = 1/sqrt(2) = -3$ dB, $angle = -45°$
...
#definition[M-Contours][
  M-contours are curves of constant closed-loop magnitude $|T(j omega)|$ on the Nichols chart, useful for assessing closed-loop performance from open-loop data.
]
...
#solution[
  ...
  $|G(j 0.1)| approx 10/sqrt(0.99^2 + 0.6^2) approx 8.5 = 18.6$ dB
  $angle approx -tan^(-1)(0.6/0.99) approx -31°$
  
  $omega = 1$:
  $G(j 1) = 10/((j 1 + 1)(j 1 + 5)) = 10/((1 - 5) + j(6))$
  $|G(j 1)| = 10/sqrt(16 + 36) = 10/sqrt(52) = 1.39 = 2.9$ dB
  $angle = -tan^(-1)(6/-4) = -123°$
  
  $omega = 10$:
  $G(j 10) = 10/((j 10 + 1)(j 10 + 5)) = 10/((-99) + j(60))$
  $|G(j 10)| = 10/sqrt(9801 + 3600) = 0.089 = -21$ dB
